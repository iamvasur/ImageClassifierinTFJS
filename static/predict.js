...
$("#model-selector").change(function(){
    ...
    $('.model-progress').html('Loading Model').show();
})

let model;
async function loadModel(name){
    model = await tf.loadLayersModel(`/${name}/model.json`, { onProgress: onprogress })
    $('.model-progress').hide();
}

let onprogress = (percentage, prefix) => {
    if (typeof prefix == 'undefined') {
       prefix = "Loading Model";
    }
    $('.model-progress').html( prefix + ' : ' +  ( 100 * percentage ).toFixed( 0 ).toString() + '%');
}


$("#predict-button").click(async function(){
    if (!$(this).hasClass("disabled")) {
	$('.predict-progress').html('Prediction Running. Please wait...').show();
	setTimeout(async function(){
    	let image= $('#selected-image').get(0);
    	let tensor = preprocessImage(image,$("#model-selector").val());
    	let prediction = await model.predict(tensor).data();
    	$('.predict-progress').hide();
